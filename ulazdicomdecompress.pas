...
uses
  Classes, SysUtils, uLazDicomDefineTypes, Graphics, Dialogs, clipbrd, ExtCtrls, lcltype;
      ...
      if lImageVoxels = (JPG.Height * JPG.Width) then begin
        ...
        for j := lHt0 - 1 downto 0 do begin
          ...
          for i := lWid0 downto 0 do begin
            lOutputBuff[lInc] := ByteRA0((lRow^[i].rgbtBlue) and 255);//lRow[i].rgbtRed;
            lOutputBuff[lInc - 1] := ByteRA0((lRow^[i].rgbtGreen) and 255);//lRow[i].rgbtRed;
            lOutputBuff[lInc - 2] := ByteRA0((lRow^[i].rgbtRed) and 255);//lRow[i].rgbtRed;
            Dec(lInc, 3);
        ...
        if lAllocSliceSz = (BMP.Height * BMP.Width) then begin
          ...
          for j := BMP.Height - 1 downto 0 do begin
            ...
            for i := (BMP.Width - 1) downto 0 do begin
              lOutputBuff[lInc] := ByteRA0((lRow^[i].rgbtRed) and 255);//lRow[i].rgbtRed;
              Dec(lInc);
            end; //for i.. each column
          end; //for j...each row
        end; //if ImageSize matches buffer size
...
label
  444;
var
  J, I, lPos, lTmpSz, lTmpPos, lImgPos, lLine, lLastPixel, lBufEntries, lAllocSliceSz, lLineStart, lLineEnd: integer;
  TmpBuff: bYTEp0;
  lByte, lByte2: byte;
  ...
  begin
    ...
    if lTmpPos > 0 then GetByte := byte(TmpBuff[lTmpPos])
    else
      GetByte := 0;
    Inc(lTMpPos);
  end;
